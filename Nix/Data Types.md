...
3. Number
	...
	Numbers can integers (like `123`) or floating point (like `123.123` or `.27e13`).
...
6. String
    ...
    You can include the results of other expressions into a string by enclosing them in `${ }`, a feature known as [string interpolation](https://nixos.org/manual/nix/stable/language/string-interpolation).
...
Indented strings support [string interpolation](https://nixos.org/manual/nix/stable/language/string-interpolation).
...
- [Number](https://nixos.org/manual/nix/stable/language/values#type-number)
    
    Numbers, which can be _integers_ (like `123`) or _floating point_ (like `123.43` or `.27e13`).
    
    See [arithmetic](https://nixos.org/manual/nix/stable/language/operators#arithmetic) and [comparison](https://nixos.org/manual/nix/stable/language/operators#comparison) operators for semantics.
    
- [Path](https://nixos.org/manual/nix/stable/language/values#type-path)
    ...
    Note that the Nix language assumes that all input files will remain _unchanged_ while evaluating a Nix expression. For example, assume you used a file path in an interpolated string during a `nix repl` session. Later in the same session, after having changed the file contents, evaluating the interpolated string with the file path again might not return a new store path, since Nix might not re-read the file contents.
...
## [List](https://nixos.org/manual/nix/stable/language/values#list)
...
## [Attribute Set](https://nixos.org/manual/nix/stable/language/values#attribute-set)

An attribute set is a collection of name-value-pairs (called _attributes_) enclosed in curly brackets (`{ }`).

An attribute name can be an identifier or a [string](https://nixos.org/manual/nix/stable/language/values#string). An identifier must start with a letter (`a-z`, `A-Z`) or underscore (`_`), and can otherwise contain letters (`a-z`, `A-Z`), numbers (`0-9`), underscores (`_`), apostrophes (`'`), or dashes (`-`).
...
Attributes can be accessed with the [`.` operator](https://nixos.org/manual/nix/stable/language/operators#attribute-selection).
...
Attribute names support [string interpolation](https://nixos.org/manual/nix/stable/language/string-interpolation):
