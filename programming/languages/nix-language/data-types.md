...
3. Finally, as a convenience, URIs can be written as is, without quotes. For instance, the string `"http://example.org/foo.tar.bz2"` can also be written as `http://example.org/foo.tar.bz2`.
**List**
...
```
Note that function calls have to be enclosed in parentheses. If they had been omitted, the result would be a list of five elements, the fourth one being a function and the fifth being a set.
...
```
Attributes can be accessed with the [`.`](https://nixos.org/manual/nix/stable/language/operators#attribute-selection) [operator](https://nixos.org/manual/nix/stable/language/operators#attribute-selection).
**Example**
...
**Example**
Both will evaluate to `"Xyzzy"` because there is no `c` attribute in the set.
```
